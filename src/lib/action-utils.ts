...
function makeDestinationPath({ data, config, genstuff }: TODO_RenameOptions) {
  return path.resolve(
    genstuff.getDestinationBasePath(),
    genstuff.renderString(normalizePath(config.path) || "", getFullData(data, config)),
  )
}

function getRenderedTemplatePath({ data, config, genstuff }: TODO_RenameOptions) {
  if (config.templateFile) {
    var absTemplatePath = path.resolve(genstuff.getGenstuffFilePath(), config.templateFile)
    return genstuff.renderString(normalizePath(absTemplatePath), getFullData(data, config))
  }

  return null
}

async function getTemplate({ data, config, genstuff }: TODO_RenameOptions) {
...
async function getRenderedTemplate({ data, config, genstuff }: TODO_RenameOptions) {
